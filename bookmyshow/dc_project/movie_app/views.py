...
def fetch_movies_api():
    ...
    for theater in theaters:
        ...
        conn.request(
            "GET", f"/theaters/detail?id={theater_id}", headers=headers)
        ...
        for mov in moviesInThisTheater:
            showtimes = mov.get('movieVariants', []) and mov['movieVariants'][0].get('amenityGroups', []) and \
                mov['movieVariants'][0]['amenityGroups'][0].get(
                    'showtimes', [])
...
def movie_theaters(request):
    if request.method == 'POST':
        ...
        theaters_with_movie = Theaters.objects.filter(
        ...
        for theater in theaters_with_movie:
            ...
            thread = Thread(target=fetch_theater_data, args=(
                theater, movies_with_name, theater_data))
        ...
        # return JsonResponse({'theater_data': theater_data, 'movie_name': movie_name})
        ...
        for mov in movies:
            if mov.movie_name not in unique_movies_name:
                unique_movies_name.append(mov.movie_name)
                movies_data.append(
                    [mov.movie_name, mov.duration, mov.image_url])
...
def movie_showtimes(request):
    if request.method == 'POST':
        ...
        for theater in theaters_with_movie:
            thread = Thread(target=process_theater, args=(
                theater, movies_with_name, theater_name, movie_name, theater_data, lock))
            threads.append(thread)
            thread.start()
...
def select_show(request):
    if request.method == 'POST':
        ...
        seat_list = []
        for book in booked_seats:
            tmp = book.seat_number_booked
            tmp = tmp.replace('seat-', '')
            tmp = tmp.replace('-', ':')

            seat_list.append(tmp)
        print(seat_list)
...
def book_seats(request):
    if request.method == 'POST':
        ...
        theater_name = request.POST.get('theater_name')
        movie_name = request.POST.get('movie_name')
        # You may need to pass the show time from the template as well
        show_time = request.POST.get('show_time')
        ...
        new_booked_seats = list(
            set(current_booked_seats.split(',')) | set(selected_seats))
        seats_instance.seat_number_booked = ','.join(
            map(str, sorted(new_booked_seats)))
        seats_instance.save()
...
def submit_selected_seats(request):
    movie_name = request.POST.get('movie_name')
    theater_name = request.POST.get('theater_name')
    show_time = request.POST.get('show_time')
    selected_seats = request.POST.get('selected_seats')
    user = request.user
    seats_booked = list(selected_seats.split(','))
    ...
    for seat in seats_booked:
        BookedSeats.objects.create(
            ...
            movie_name=movie_name,
            show_times=show_time,
            seat_number_booked=seat,
        )
    # Return a response
    print("\n\nSelected seats submitted successfully\n\n")
    return HttpResponseRedirect(reverse("index"))


def show_selected_show(request):
    movie_name = request.GET.get('movie_name')
    theater_name = request.GET.get('theater_name')
    show_time = request.GET.get('show_time')

    # Get booked seats for the selected movie, theater, and show time
    booked_seats = BookedSeats.objects.filter(
        movie_name=movie_name,
        theater_name=theater_name,
        show_times=show_time
    ).values_list('seat_number_booked', flat=True)
